...
const Admin = () => {
    ...
    const [newProject, setNewProject] = useState({ title: '', category: '', image: '', tags: [] });
    ...
    const handleSavePost = async (e) => {
        ...
        const trySave = () => {
            try {
                ...
                setNewPost({ title: '', slug: '', metaDescription: '', content: '', image: '', date: new Date().toLocaleDateString('pt-BR') });
                return true;
            } catch (e) {
    ...
    const handleSaveProject = async (e) => {
        ...
        const trySave = () => {
            try {
                ...
                setNewProject({ title: '', category: '', image: '', tags: [] });
                return true;
            } catch (e) {
                console.error("Storage Error:", e);
                return false;
                            ...
                            <form onSubmit={handleSaveProject} className="admin-form">
                                ...
                                <div className="form-group">
                                    ...
                                    <div className="upload-container">
                                        ...
                                        {newProject.image && (
                                            <div className="upload-preview">
                                                <AsyncImg src={newProject.image} alt="Preview" />
                                                <button type="button" onClick={() => setNewProject({ ...newProject, image: '' })} className="remove-img"><X size={14} /></button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="form-group">
                                    <label>Tags do Projeto (separadas por vírgula)</label>
                                    <input
                                        type="text"
                                        value={newProject.tags.join(', ')}
                                        onChange={(e) => setNewProject({ ...newProject, tags: e.target.value.split(',').map(t => t.trim()).filter(t => t) })}
                                        placeholder="Ex: React, E-commerce, Responsivo"
                                    />
                                </div>
